"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.SuperadminController = void 0;
const common_1 = require("@nestjs/common");
const swagger_1 = require("@nestjs/swagger");
const jwt_auth_guard_1 = require("../auth/guards/jwt-auth.guard");
const roles_guard_1 = require("../auth/guards/roles.guard");
const roles_decorator_1 = require("../auth/guards/roles.decorator");
const client_1 = require("@prisma/client");
const superadmin_service_1 = require("./superadmin.service");
const superadmin_dto_1 = require("./dto/superadmin.dto");
let SuperadminController = class SuperadminController {
    superadminService;
    constructor(superadminService) {
        this.superadminService = superadminService;
    }
    async getAllStaff() {
        return this.superadminService.getAllStaff();
    }
    async createStaff(dto) {
        return this.superadminService.createStaffUser(dto);
    }
    async updateRole(id, dto) {
        return this.superadminService.updateUserRole(id, dto);
    }
    async deactivateStaff(id) {
        return this.superadminService.deactivateStaffUser(id);
    }
};
exports.SuperadminController = SuperadminController;
__decorate([
    (0, common_1.Get)('staff'),
    (0, swagger_1.ApiOperation)({ summary: 'Get all staff users' }),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", Promise)
], SuperadminController.prototype, "getAllStaff", null);
__decorate([
    (0, common_1.Post)('staff'),
    (0, swagger_1.ApiOperation)({ summary: 'Create new staff user' }),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [superadmin_dto_1.CreateStaffDto]),
    __metadata("design:returntype", Promise)
], SuperadminController.prototype, "createStaff", null);
__decorate([
    (0, common_1.Put)('staff/:id/role'),
    (0, swagger_1.ApiOperation)({ summary: 'Update staff user role' }),
    __param(0, (0, common_1.Param)('id')),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, superadmin_dto_1.UpdateRoleDto]),
    __metadata("design:returntype", Promise)
], SuperadminController.prototype, "updateRole", null);
__decorate([
    (0, common_1.Delete)('staff/:id'),
    (0, swagger_1.ApiOperation)({ summary: 'Deactivate staff user' }),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], SuperadminController.prototype, "deactivateStaff", null);
exports.SuperadminController = SuperadminController = __decorate([
    (0, swagger_1.ApiTags)('SuperAdmin'),
    (0, swagger_1.ApiBearerAuth)(),
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard, roles_guard_1.RolesGuard),
    (0, roles_decorator_1.Roles)(client_1.Role.SUPERADMIN),
    (0, common_1.Controller)('superadmin'),
    __metadata("design:paramtypes", [superadmin_service_1.SuperadminService])
], SuperadminController);
//# sourceMappingURL=superadmin.controller.js.map